function a_final = repeat_orbit_2(i,e,j,k)
    % Calculate the semi major axis for a repeat orbit with a given
    % inclination(i), eccentricity (e), orbits(j) and days(k) with the J2
    % effect in directions of flight ignored
    i = deg2rad(i);
    J2 = 1082.63*10^(-6); 
    Re = 6378136; %m
    ...
    while dif > 0.0001
        ...
        n = j./k .*(L_dot - RAAN_dot); %deg/sid_day
